...
import VagaCarrossel from "../../components/vaga/VagaCarrossel";
...
export default function Vagas({ navigation }) {
  ...
  const handleVerMais = async () => {
    ...
    try {
      ...
      const pg = paginaVagas(listaCompleta, {
        page: nextPage,
        pageSize: 6,
      });
  ...
  const goDetalhe = (v) =>
    navigation.navigate("DetalheVaga", { id: v.id, vaga: v });
      ...
      >
        <View style={{ paddingTop: 12, paddingBottom: 8 }}>
          ...
          <View style={{ marginHorizontal: -H_PADDING }}>
            <VagaCarrossel navigation={navigation} />
          </View>
