...
import { styles as ns } from "../../styles/news/DetalheNoticiaStyles"; // estilos de comentários
...
// DEBUG: garante que os imports chegaram como função
console.log("🐛 [DEBUG imports DetalheVaga]:", {
  listComentariosVaga: typeof listComentariosVaga,
  criarComentarioVaga: typeof criarComentarioVaga,
});
...
export default function DetalheVaga({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        if (vagaParam) {
          console.log("🧩 [VAGA] via params:", vagaParam?.id);
          if (live) setVaga(vagaParam);
        } else if (idParam != null) {
          console.log("📤 [REQ] GET /vagas/", idParam);
          const { data } = await api.get(`/vagas/${idParam}`);
          console.log("✅ [RESP] /vagas/:id ->", data?.id);
          if (live) setVaga(data);
        } else {
          throw new Error("Parâmetros inválidos para abrir a vaga.");
        }
      } catch (e) {
        console.log("❌ [ERRO carregar vaga]:", e?.message || e);
  ...
  useEffect(() => {
    if (vaga) {
      console.log("🧾 [STATE vaga]:", JSON.stringify(vaga, null, 2));
    }
  }, [vaga]);
  ...
  const onPressWhats = useCallback(async () => {
    console.log("🔗 [WHATS] abrir:", whatsappHref);
    if (!whatsappHref) {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (vaga?.id) {
          console.log("📤 [REQ] listar comentários (vaga):", vaga.id);
          const lista = await listComentariosVaga(vaga.id);
          console.log("✅ [RESP] comentários (vaga):", lista.length);
          setComentarios(lista);
        }
      } catch (err) {
        console.log("❌ [ERRO listar comentarios vaga]:", err?.message || err);
        setComentarios([]);
      }
    })();
  }, [vaga?.id]);

  // Lista ao abrir seção
  useEffect(() => {
    (async () => {
      if (showComentarios && vaga?.id) {
        setLoadingComentarios(true);
        try {
          console.log("📤 [REQ] listar comentários (abrir seção):", vaga.id);
          const lista = await listComentariosVaga(vaga.id);
          console.log("✅ [RESP] comentários (abrir seção):", lista.length);
          setComentarios(lista);
        } catch (err) {
          console.log("❌ [ERRO listar comentarios seção]:", err?.message || err);
          setComentarios([]);
        } finally {
  ...
  const enviarComentario = useCallback(async () => {
    try {
      ...
      console.log("🧪 [DEBUG comentar] userId:", userId, "token? ", !!token);
      ...
      console.log("📤 [REQ] criarComentarioVaga payload:", {
      ...
      console.log("✅ [RESP] criarComentarioVaga:", criado?.id ?? criado);

      Alert.alert("Sucesso", "Comentário publicado!");
      setNovoComentario("");
      setComentarios((prev) => [...prev, criado]);
    } catch (e) {
      console.log("❌ [ERRO criarComentarioVaga]:", e?.message || e, e?.response?.data);
      Alert.alert(
        "Erro",
  ...
  const carregarRelacionadas = useCallback(async (baseId, zonaAtual) => {
    setRelLoading(true);
    try {
      console.log("📤 [REQ] getTodasVagas (relacionadas) baseId:", baseId, "zona:", zonaAtual);
      ...
      console.log("✅ [RESP] relacionadas:", pg.items.length, "hasMore:", pg.hasMore);
      setRelItens(pg.items);
      setRelState({ page: 1, pageSize: 6, hasMore: pg.hasMore });
    } catch (err) {
      console.log("❌ [ERRO relacionadas]:", err?.message || err);
      setRelItens([]);
  ...
  const verMaisRelacionadas = useCallback(async () => {
    console.log("📤 [REQ] verMaisRelacionadas page:", relState.page + 1);
    const data = await getTodasVagas();
    const filtradas = (data || [])
    ...
    console.log("✅ [RESP] verMaisRelacionadas items:", pg.items.length, "hasMore:", pg.hasMore);
