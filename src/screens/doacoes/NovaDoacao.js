...
// 🔹 Mapeia nome da região para enum do backend
function mapRegiaoToEnum(regiao) {
  const map = {
    ...
    sudoeste: "SUDOESTE",
    sudeste: "SUDESTE",
    noroeste: "NOROESTE", // 🔹 corrigido: não existe NOROESTE2 no backend
  };
...
export default function NovaDoacao({ navigation }) {
  ...
  const enviar = useCallback(async () => {
    ...
    try {
      ...
      console.log("❌ criarDoacao:", e?.message || "erro");
      Alert.alert("Erro", "Falha ao criar a doação.");
    } finally {
      setLoading(false);
    }
  }, [titulo, descricao, telefone, zona, img, navigation]);
      ...
      >
        <ScrollView contentContainerStyle={{ padding: 16 }}>
          ...
          <View style={{ padding: 4 }}>
            ...
            >
              <Text style={{ color: "#111827" }}>{String(zona).toLowerCase()}</Text>
            </TouchableOpacity>
